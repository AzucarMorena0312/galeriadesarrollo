...
// Accediendo al boton de nueva Imagen
const btnNuevo = document.querySelector('#btnNuevo');
btnNuevo.addEventListener('click', () => {
    //alert("¿que pompo?")
    window.location.href = "http://localhost/galeriadesarrollo/templates/formulario.php";
})
...
const modalFmAutor = document.querySelector('#modal_autor');
 ...
 function listarImagenes(){
    ...
    .then(response => {
        ...
        response.forEach(elemento => {
            ...
            let url = 'http://localhost/galeriadesarrollo/upload/' + elemento.imagen;
            str += "<img src='" + url + "' alt='Imagen Producto'>";

            str += "<h4>" + elemento.nombre + "</h4>";
            str += "<h4>" + elemento.autor + "</h4>";
            str += "<p>" + elemento.descripcion + "<p>";
            str += "<div class='botones'>";
            str += "<a href='#' class='btn_yellow' onclick=actualizarImagen('" + elemento.id + "') >Editar</a><a href='#' class='btn_red' onclick=eliminarImagen('" + elemento.id + "') >Borrar</a>";
            str += "</div>"
            str += "</div>";
        });
        str += "</div>";
...
function actualizarImagen(id){
    ...
    .then(response => {
        ...
        modalFmAutor.value = response.autor;
...
// Invocamos Funcion
listarImagenes(); // <== ESTE ERA EL ERROR
